...
def genPoint(gaussian = 0):
    # x = random.uniform(0,1)*XMAX
    # y = random.uniform(0,1)*YMAX
...
def lineFromPoints(p1,p2):
    ...
    for i in range(2):  # each dimension
...
def pointPointDistance(p1,p2):
    ...
    # for i in range:  # each dimension, general case
    #     h = p2[i] - p1[i] 
    #     llsq = llsq + (h*h)
    # return math.sqrt(llsq)
...
def getEndPoints(xy):
    start = [xy[0]-(length/2)*math.cos(math.radians(xy[2])),xy[1]-(length/2)*math.sin(math.radians(xy[2]))]
    end = [xy[0]+(length/2)*math.cos(math.radians(xy[2])),xy[1]+(length/2)*math.sin(math.radians(xy[2]))]
    return start, end

def calculateStep(p1, p2):
    step = SMALLSTEP
    if (pointPointDistance(p1,p2)<step):
        step = pointPointDistance(p1, p2)
    return (step)

def hitsObstacles(to, random, obstacles=obstacles, dilation=1):
    for ob in obstacles:
        if (inRect(to, ob, dilation)):
            return 1
        if(lineHitsRect(to, random, ob )):
            return 1
    return 0

def calculateTheta(diff, xy_near_theta):
    ans = xy_near_theta
    if abs(diff) > SMALLTHETASTEP:
        if diff > 0: 
            ans = xy_near_theta + SMALLTHETASTEP
...
def rrt_search(G, tx, ty, ttheta):
    ...
    while 1:
        ...
        diff = xy_rand[2] - xy_near[2]

        dist = pointPointDistance(xy_near, xy_rand)

        new_theta = calculateTheta(diff, xy_near[2])
        
        xy_new = [xy_rand[0], xy_rand[1], new_theta]

        step = calculateStep(xy_near, xy_new)

        normal = lineFromPoints(xy_near, xy_new)

        xy_new = [xy_near[0] + (normal[0] * step), 
        xy_near[1] + (normal[1] * step), new_theta]

        new_st, new_end = getEndPoints(xy_new)
        near_st, near_end = getEndPoints(xy_near)

        if (xy_new[0] <length or xy_new[0]> (XMAX-length) or 
            xy_new[1] <length or xy_new[1]> (YMAX-length)): 
            continue
        if (hitsObstacles(new_st, near_st) or 
            hitsObstacles(new_end, near_end) or 
            hitsObstacles(new_end, new_st) or 
            hitsObstacles(near_st, near_end)): continue
        else:
            new_v = pointToVertex(xy_new)
            G[nodes].append(new_v)
            G[edges].append((closest,new_v))
            canvas.polyline([vertices[closest], vertices[new_v]], style=0)
